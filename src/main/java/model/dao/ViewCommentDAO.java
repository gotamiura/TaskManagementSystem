...
public class ViewCommentDAO {
	...
	public List<EnterCommentsBean> showComments(int taskId, String userId) throws ClassNotFoundException, SQLException {

		List<EnterCommentsBean> commentList = new ArrayList<>();
		try (Connection con = ConnectionManager.getConnection();
				PreparedStatement pstmt = con.prepareStatement(
						"SELECT c.comment_id, c.task_id, t.task_name, c.user_id, u.user_name, c.comment, c.update_datetime FROM t_comment c left join t_task t on c.task_id = t.task_id left join m_user u on c.user_id = u.user_id WHERE c.task_id = ? and c.user_id = ?")) {
